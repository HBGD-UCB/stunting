...
d <- d %>% filter(intervention_variable!="enstunt" & intervention_variable!="perdiar24"  & intervention_variable!="predfeed3"  & intervention_variable!="predfeed36"  & intervention_variable!="predfeed6")
...
d <- relevel_RF(d, variable="enwast", reference="No")
...
ggsave(hm2, file="Stunting_RF_heatmap.png", width=9.2, height=5.6)
...
ggsave(hm3, file="Stunting_RF_heatmap_greyNA.png", width=9.2, height=5.6)


#Drop insignificant
RMAestfull_sig <- RMAestfull %>% group_by(intervention_variable) %>% filter(min(Pvalue) < 0.05)
RMAestfull_sig <- droplevels(RMAestfull_sig)
...
ggsave(hm4, file="Stunting_RF_heatmap_justsig.png", width=9.2, height=5.6)
...
ggsave(hm5, file="Stunting_RF_heatmap_justsig_lable.png", width=9.2, height=5.6)
