...
#Subset to pre-birth exposures
unique(d$intervention_variable)
d <- d[d$intervention_variable %in% c("gagebrth",              "vagbrth",       "hdlvry",              "mage",          "fage",          "mhtcm",        
                                      "mwtkg",         "mbmi",          "single",        "fhtcm",         "nrooms",        "nchldlt5",      "parity",       
                                      "meducyrs",      "feducyrs",      "hfoodsec",        "cleanck",       "impfloor",      "impsan",  
                                      "hhwealth_quart",
                                      "safeh20"),]

#Drop jivita-3 hhwealth, which is giving errors
d <- d %>% filter(!(d$intervention_variable=="hhwealth_quart" & (d$studyid=="kiGH5241-JiVitA-3" | d$studyid=="kiGH5241-JiVitA-4")))

                  
                  
#Check ratio of largest to smallest SE
df <- d %>% group_by(intervention_variable, agecat, intervention_level) %>%
  summarize(R=max(untransformed_se)/min(untransformed_se)) %>% as.data.frame()
df

#df <- d[d$intervention_variable=="hhwealth_quart",]
#d <- d %>% filter(untransformed_se < 0.5)
...
RMAest <- d %>% group_by(outcome_variable, intervention_variable, intervention_level) %>% 
...
pdf("Birth prev/Risk Factor Plots Birth Stunting.pdf", height=8, width=12)
...
for(i in levels(RMAest$intervention_variable)){
  ...
  p <-  ggplot(plotdf, aes(x=intervention_level)) + 
    geom_point(aes(y=RR, fill=outcome_variable, color=outcome_variable), size = 4) +
    geom_linerange(aes(ymin=RR.CI1, ymax=RR.CI2, color=outcome_variable),
                   alpha=0.5, size = 3) +
    ...
    scale_fill_manual(values=rep(tableau10,4), drop=TRUE, limits = levels(RMAest$outcome_variable)) +
    scale_colour_manual(values=rep(tableau10,4), drop=TRUE, limits = levels(RMAest$outcome_variable)) +
    scale_size_continuous(range = c(0.5, 1))+
    theme(strip.background = element_blank(),
          legend.position="none",
          strip.text.x = element_text(size=12),
          axis.text.x = element_text(size=12, angle = 45, hjust = 1)) +
    facet_wrap(~outcome_variable, nrow = 1, labeller = label_wrap) +
    ggtitle(plotdf$RFlabel[1])
  ggsave(p, file=paste0("Birth prev/",j,"_",i,"_RFplot.png"), width=5.7, height=4.6)
  print(p)
}
