...
library(FField)
library(viridis)
setwd("C:/Users/andre/Documents/HBGDwasting/R/andrew/country_centroids")
...
load("C:/Users/andre/Documents/HBGDki/Results/stunt_scattermap_data.Rdata")
...
summary(d$n)
d <- d %>% mutate(ncat=ntile(n,4), Stunted=Stunted*100) %>% rename(`Number of observations`=n, `Stunting Prevalence (%)`=Stunted) 


p +   geom_point(aes(x = x, y = y, size = `Number of observations`, color=`Stunting Prevalence (%)`), data = d, alpha = 0.8) +
...
#Stunting Webinar format

p <- ggplot(world, aes(long, lat)) +
  geom_map(map=world, aes(map_id=region), fill=NA, color="grey20") +
  coord_quickmap() + theme_bw() + coord_cartesian(xlim=c(-90,120), ylim=c(-36,50)) +
  geom_point(aes(x = x, y = y, size = `Number of observations`, color=`Stunting Prevalence (%)`), data = d, alpha = 0.8) +
...
ggsave(p, file="C:/Users/andre/Dropbox/HBGDki figures/Stunting Webinar/Stunting_map.png", width=9, height=4)
